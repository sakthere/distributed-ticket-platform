...
- **No test coverage yet.** Elevated from a background item to an explicit next phase (`Phase 1.5 - Testing Foundation` in `ROADMAP.md`) — unit tests for the Application-layer handlers, integration tests for the API layer, before Ticket Management work begins.
